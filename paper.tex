...
We implement \gralcrag{} as an open-source framework and evaluate it across a document-length gradient on 2{,}359 IMRaD-filtered PubMed Central articles with six retrieval strategies and 2{,}033 template-based cross-section questions. Our experiments reveal that while content-similarity methods dominate point-estimate MRR, \gralcrag{} retrieves from significantly more document sections---an advantage that scales with document length and is invisible to standard ranking metrics.
...
Our full-text evaluation reveals that \gralcrag{}'s value is \textbf{context-dependent} and \textbf{dimension-dependent}. Semantic chunking achieves the highest MRR across all document-length conditions (0.488 on introduction, 0.555 on partial, 0.517 on full-text; Table~\ref{tab:fulltext}), demonstrating that content-similarity remains the strongest signal for point-estimate retrieval accuracy. However, content-only methods are structurally blind: Naive, Semantic, and Late Chunking always retrieve from a single section (SecCov@5\,=\,1.0), regardless of document length. In contrast, \gralcrag{} (KG) retrieves from 2.33 sections on introduction-only documents and 4.46 sections on full-text articles at $k$=5---scaling to 15.57 at $k$=20 (Table~\ref{tab:crosssection}). This reveals that \gralcrag{}'s advantage lies in \emph{what} it retrieves (structural diversity across document sections) rather than ranking accuracy alone.
...
Key extensions include: adaptive $\lambda$ and $\beta$ weighting based on document length to reduce KG noise on shorter documents; section-diversity re-ranking objectives to improve cross-section recall (which remains 0.000 even at $k$=20); hybrid strategies that combine semantic chunking's MRR strength with \gralcrag{}'s structural coverage; multi-granularity retrieval; cross-document citation graph linking; and integration with temporal knowledge graph evolution~\cite{zhang2025medkgent}.

% ══════════════════════════════════════════════════════════════
\section{Conclusion}

We presented \gralcrag{}, a framework integrating graph-aware structural intelligence into the late chunking paradigm for biomedical RAG. Evaluation on 2{,}359 IMRaD-filtered PMC articles across a document-length gradient with 2{,}033 cross-section questions reveals that \gralcrag{} retrieves from significantly more document sections than content-only methods---up to 15.6$\times$ on full-text articles (SecCov@20 = 15.57 vs.\ 1.0)---even though content-similarity methods (semantic chunking) achieve higher point-estimate MRR (0.517 vs.\ 0.323).

These findings yield three actionable insights: (1)~MRR alone is insufficient for evaluating full-text retrieval---section coverage reveals structural diversity that ranking metrics miss; (2)~retrieval depth amplifies structural advantages, with \gralcrag{} scaling from 4.5$\times$ coverage at $k$=5 to 15.6$\times$ at $k$=20 on full-text articles, while content-only methods remain flat at 1.0; and (3)~the choice between content-similarity and structure-aware methods should be guided by downstream task requirements---ranking accuracy favors semantic chunking, while evidence breadth favors \gralcrag{}. We release the complete \gralcrag{} codebase to enable reproduction and extension.
