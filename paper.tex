...
Our experiments reveal a surprising finding: the methods that achieve the highest ranking accuracy retrieve from only a single document section, while structure-aware methods that rank lower on MRR surface evidence from up to 15.6$\times$ more sections. Preliminary generation experiments further reveal that current LLMs do not yet capitalize on this broader evidence, suggesting that multi-section synthesis---not just retrieval---is a key bottleneck for biomedical RAG.
...
\multirow{6}{*}{Introduction}
  & Naive              & 0.398 & 0.327 & 0.480 & 0.648 \\
  & Semantic           & \textbf{0.488} & \textbf{0.430} & \textbf{0.553} & \textbf{0.921} \\
  & Late Chunking      & 0.379 & 0.313 & 0.465 & 0.648 \\
  & Structure-Aware    & 0.393 & 0.324 & 0.477 & 0.706 \\
  & \gralcrag{} (KG)   & 0.364 & 0.290 & 0.451 & 0.654 \\
  & \gralcrag{} (+Graph) & 0.347 & 0.271 & 0.436 & 0.612 \\
\midrule
\multirow{6}{*}{Partial}
  & Naive              & 0.361 & 0.288 & 0.453 & 0.630 \\
  & Semantic           & \textbf{0.555} & \textbf{0.489} & \textbf{0.637} & \textbf{0.988} \\
  & Late Chunking      & 0.339 & 0.267 & 0.422 & 0.616 \\
  & Structure-Aware    & 0.387 & 0.320 & 0.469 & 0.750 \\
  & \gralcrag{} (KG)   & 0.339 & 0.266 & 0.429 & 0.644 \\
  & \gralcrag{} (+Graph) & 0.358 & 0.282 & 0.454 & 0.664 \\
\midrule
\multirow{6}{*}{Full-text}
  & Naive              & 0.344 & 0.268 & 0.435 & 0.788 \\
  & Semantic           & \textbf{0.517} & \textbf{0.448} & \textbf{0.607} & \textbf{1.100} \\
  & Late Chunking      & 0.326 & 0.263 & 0.395 & 0.744 \\
  & Structure-Aware    & 0.365 & 0.296 & 0.446 & 0.879 \\
  & \gralcrag{} (KG)   & 0.323 & 0.261 & 0.397 & 0.755 \\
  & \gralcrag{} (+Graph) & 0.332 & 0.269 & 0.407 & 0.766 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Generation quality on 50 cross-section questions (full-text condition, GPT-4o-mini). Section diversity measures the average number of distinct sections in the top-5 retrieved chunks. Higher diversity does not yet translate to higher F1, revealing a gap between structural retrieval and multi-section synthesis.}
...
Semantic chunking achieves the highest F1 (0.403) while retrieving from a single section, whereas Structure-Aware achieves comparable F1 (0.381) while drawing from 4.48 sections on average. The small F1 gap ($\Delta$\,=\,0.022) suggests that GPT-4o-mini produces slightly more focused answers from single-section context but does not effectively leverage the diverse multi-section evidence that structure-aware retrieval provides. This indicates that \textbf{multi-section synthesis is an unsolved bottleneck}: the retrieval system surfaces structurally diverse evidence, but current generation models are not designed to integrate it.
...
Our generation experiments (Section~5.6) reveal an important negative result: despite retrieving from 4.5$\times$ more document sections, structure-aware methods achieve comparable or slightly lower F1 than content-only methods (0.381 vs.\ 0.403). This suggests a \textbf{retrieval-to-generation gap}---structure-aware retrieval surfaces diverse, multi-section evidence that current LLMs fail to effectively synthesize. GPT-4o-mini produces slightly more focused answers from single-section context, likely because concentrated evidence is easier to extract from than distributed evidence spanning multiple sections. This gap represents a key open problem: the retrieval component is providing structurally diverse context, but the generation component is not designed to integrate it. Future work on generation-side innovations---multi-section chain-of-thought prompting, section-aware attention, or explicit evidence aggregation---may be needed to unlock the value of structural retrieval.
